...
object AudioPreprocessor {
    ...
    /**
     * Compute 2D MFCC feature array shaped (nFrames, nMfcc) for TFLite model input.
     */
    fun computeMFCC(
        audio: FloatArray,
        ...
        val frameSize = maxOf(1, audio.size / nFrames)
        return Array(nFrames) { f ->
            val start = f * frameSize
            val end   = minOf(start + frameSize, audio.size)
            val frame = if (end > start) audio.sliceArray(start until end) else floatArrayOf(0f)
            computeBandEnergies(frame, nMfcc)
        }
    }
